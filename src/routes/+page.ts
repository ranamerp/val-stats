...
export const load: PageLoad = async ({ fetch, parent }) => {
    ...
    const statsPromise = (async () => {
        ...
        let puuids: { puuid: string, regn: string }[] | null = null;
        ...
        try {
            ...
            puuids = playerlist?.map((p: { puuid: string, regn: string }) => ({ puuid: p.puuid, regn: data?.current_match.region })) ?? [];
            if (error) {
        ...
        if (puuids) {
            // Go through it, if 
            for (const { puuid, regn } of puuids) {
                matchResponse = await fetch(`/api/match?region=${regn ?? 'na'}&puuid=${puuid}`);
        ...
        if (matchResponse?.status === 500) {
            ...
            name = player ? player.name + '#' + player.tag : name;
